...
El plasmoide puede mostrar tus eventos de Google Calendar como **bloques
...
Se prende/apaga con el **botón de tilde arriba a la derecha** del popup
(o desde Configurar → Google Calendar).
...
key sola no alcanza). Como el plasmoide es QML puro y no puede levantar un
...
1. Autorizás **una sola vez** ingresando un código corto en una página de
   Google.
2. Google devuelve un **refresh token** que el plasmoide guarda.
3. En cada sincronización, el plasmoide cambia ese refresh token por un
...
```bash
journalctl --user -f _COMM=plasmashell | grep '\[GoogleCal\]'
```

El botón **ⓘ** del popup también muestra el log de Google junto al de
Jira y Clockify.
