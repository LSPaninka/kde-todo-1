 ...
 * stored in the parallel StringLists jira2StatusNames / jira2StatusColors.
...
ColumnLayout {
    ...
    function _name(i)  { var v = (page._rev, plasmoid.configuration.jira2StatusNames  || [])[i]; return v !== undefined ? v : ""; }
    function _color(i) { var v = (page._rev, plasmoid.configuration.jira2StatusColors || [])[i]; return (v !== undefined && v !== "") ? v : _defaultColor; }

    function _setName(i, value) {
        var arr = (plasmoid.configuration.jira2StatusNames || []).slice();
        while (arr.length < _slots) arr.push("");
        arr[i] = value;
        plasmoid.configuration.jira2StatusNames = arr;
        page._rev++;
    }
    function _setColor(i, value) {
        var arr = (plasmoid.configuration.jira2StatusColors || []).slice();
        while (arr.length < _slots) arr.push("");
        arr[i] = value;
        plasmoid.configuration.jira2StatusColors = arr;
        page._rev++;
    }
